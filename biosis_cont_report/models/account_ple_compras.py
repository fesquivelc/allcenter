...
class PleCompras(models.Model):
    _name = 'account.ple.compras'
    ...
    def get_ple_compras(self, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        if len(invoices_old) > 0:
            ple_modificados = self.update_ple_items_compras(invoices_old, fecha_fin)
    ...
    @api.multi
    def update_ple_items_compras(self, invoices, fecha_fin):
        ple_items = ''
        for invoice in invoices:
            ...
            if ple_actual.create_date < invoice.write_date:
                ...
                if ple_actual.fecha_v_5 != datetime.datetime.strptime(invoice.date_due, '%Y-%m-%d').strftime('%d/%m/%Y') and ple_actual.fecha_v_5 != '01/01/0001':
                    flag_change_invoice = True
                if ple_actual.tipo_cpbt_6 != invoice.tipo_documento.code:
                    flag_change_invoice = True
                if ple_actual.serie_cpbt_7 != invoice.numero.split('-')[0]:
                    flag_change_invoice = True
                if ple_actual.numero_cpbt_9 != invoice.numero.split('-')[1]:
                    flag_change_invoice = True
                ...
                if invoice.invoice_id:
                    if ple_actual.serie_cpbt_mod_28 != invoice.invoice_id.numero.split('-')[0] and ple_actual.serie_cpbt_mod_28 !=  u'-':
                        flag_change_invoice = True
