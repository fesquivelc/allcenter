...
class PleBalanceComprobacion(models.Model):
    _name = 'account.ple.balancecomprobacion'
    ...
    def get_ple_bc(self, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        bc_report = self.get_lines_report(fecha_inicio, fecha_fin) #Lineas de bc segun el periodo ingresado

        bc_ple_list = self.env['account.ple.balancecomprobacion'].search([
        ...
        if len(bc_ple_new) > 0:
            ...
            """
            ple_nuevos = self.create_ple_items_bc(bc_ple_new, fecha_reporte, fecha_inicio, fecha_fin)
        ...
        if len(bc_ple_list) > 0:
            """
             ...
             """
            ple_old = self.update_ple_items_bc(bc_ple_old, bc_ple_update, fecha_reporte, fecha_inicio, fecha_fin)
    ...
    @api.multi
    def create_ple_items_bc(self, account_lines, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        ple_bc = self.env['account.ple.balancecomprobacion']
    ...
    @api.multi
    def update_ple_items_bc(self, bc_ple_old, bc_ple_update, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        for line in bc_ple_old:
            ple_item = self.env['account.ple.balancecomprobacion'].search([
                ('codigo_cuenta_2','=',line['codigo'])
            ],limit=1)
            ple_items = ple_items + ple_item.get_ple_line()
    ...
    def get_lines_report(self, fecha_inicio, fecha_fin):
        lines = []
        ...
        report_lines = reporte_balance.get_account_lines(self.get_data_report(libro_electronico, fecha_inicio, fecha_fin))
    ...
    def get_data_report(self, libro_electronico, fecha_inicio, fecha_fin):
        data = {}
