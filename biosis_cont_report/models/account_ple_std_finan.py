...
class PleEstadoSituacionFinanciera(models.Model):
    _name = 'account.ple.estado.situacion.finaciera'
    ...
    def get_ple(self, fecha_reporte, fecha_inicio, fecha_fin):
        ...
        ef_ple_list = self.env['account.ple.estado.situacion.finaciera'].search([
        ...
        if len(ef_ple_new) > 0:
            ...
            """
            ple_nuevos = self.create_ple_items(ef_ple_new, fecha_reporte, fecha_inicio, fecha_fin)
        ...
        if len(ef_ple_list) > 0:
            """
             ...
             """
            ple_old = self.update_ple_items(ef_ple_old, ef_ple_update, fecha_reporte, fecha_inicio, fecha_fin)
            ple_ef_res = ple_ef_res + ple_old

        return ple_ef_res

    @api.multi
    def create_ple_items(self,ef_ple_new,fecha_reporte,fecha_inicio,fecha_fin):
        ple_items = ''
        ple_ef = self.env['account.ple.estado.situacion.finaciera']
    ...
    @api.multi
    def update_ple_items(self,ef_ple_old, ef_ple_update, fecha_reporte, fecha_inicio, fecha_fin):
        ple_items = ''
        for line in ef_ple_old:
            ple_item = self.env['account.ple.estado.situacion.finaciera'].search([
    ...
    def get_lines_report(self, fecha_inicio, fecha_fin):
        ...
        report_lines = reporte_balance.get_account_lines(
            self.get_data_report(libro_electronico, fecha_inicio, fecha_fin))
