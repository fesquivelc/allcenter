...
class AccountInvoice(models.Model):
    ...
    tipo_documento = fields.Many2one('einvoice.catalog.01', string=u'Tipo Documento',  states={'draft': [('readonly', False)]})
    tipo_nota_credito = fields.Many2one('einvoice.catalog.09', string=u'Tipo Nota de Crédito', states={'draft': [('readonly', False)]})
    tipo_nota_debito = fields.Many2one('einvoice.catalog.10', string=u'Tipo Nota de Débito', states={'draft': [('readonly', False)]})
    ...
    pagina_detraccion = fields.Boolean(string='Constancia Depósito Detraccioón', default=False,  states={'draft': [('readonly', False)]})
    numero_detraccion = fields.Char(string=u'Número', index=True, help='Ingrese el numero de referencia del comprobante de detraccion')
    fecha_emision_detraccion = fields.Date(string='Fecha de Emisión', index=True)
    #Monto de la detraccion a cobrar o a pagar
    monto_detraccion = fields.Monetary(string=u'Monto Detracción', store=True, help="Monto a pagar de detraccion",  states={'draft': [('readonly', False)]})
    ...
    def action_move_create(self):
        ...
        if (self.tipo_documento.id == 8 or self.tipo_documento.id == 9) or (self.invoice_line_ids.product_id.detraccion == True):
            ...
            if self.invoice_line_ids.product_id.asset_category_id.ids > 0:
                res = super(AccountInvoice, self).action_move_create()
                return res
            else:
                if invoice_line_gastos_ids > 0:
                    self.asiento_gastos(invoice_line_gastos_ids)
                else:
                    res = super(AccountInvoice, self).action_move_create()
                    return res
    ...
    def get_taxes_values(self):
        ...
        for line in self.invoice_line_ids:
            ...
            if len(taxes) == 2:
                taxes[0]['amount'] = (taxes[0]['base'] + (taxes[1]['base'] * (igv_compras.amount / 100))) * (self.invoice_line_ids.product_id.tipo_percepcion.amount / 100)
        ...
        if self.type == 'in_invoice':
            if self.monto_detraccion != False:
                if residual == 0.0:
                    ...
                    self.residual_company_signed = round((abs(residual_company_signed) * sign) - self.monto_detraccion,2)
...
class AccountInvoiceLine(models.Model):
    ...
    @api.one
    @api.depends('value_tipo_cambio','price_unit_dolars', 'price_unit', 'discount', 'invoice_line_tax_ids', 'quantity', 'operacion',
                 'product_id', 'invoice_id.partner_id', 'invoice_id.currency_id', 'invoice_id.company_id','invoice_id.monto_detraccion')
        ...
        if porcentaje > 0:
            if porcentaje != False:
                if taxes['total_included'] > monto_detraccion_producto:
                  val = taxes['total_included'] * (porcentaje/100)
                  if self.invoice_id.monto_detraccion != val:
                    self.invoice_id.monto_detraccion = 0.0
                    self.invoice_id.monto_detraccion = round(val,2)
                  self.invoice_id.residual = round(taxes['total_included'] - (taxes['total_included'] * (porcentaje/100)),2)
